...
export class CheckCalcScorer extends Scorer {
  /**
   * コンストラクタ
  ...
  /**
   * 反応時間の平均
   * @param {number} lastDiff - 最後に出題されてから終了までの時間
   */
  getAveResponseTime(lastDiff){
    let responseTimes = [];
    this.metric.answers.forEach((v, i, a)=>{
      if (v === true) {
        responseTimes.push(this.metric.responseTimes[i]);
      } else {
        responseTimes.push(this.metric.responseTimes[i]<500 ?500 :this.metric.responseTimes[i]);
      }
    });
    responseTimes.push(lastDiff);
    return responseTimes.reduce((pre, curr, i) => {
      return pre + curr;
